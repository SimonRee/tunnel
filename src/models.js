...
export function setLoadingManager(manager) {
  ...
  DRLoader.setDecoderPath('/draco/'); // assicurati che i file decoder siano in /draco/

loader = new GLTFLoader(sharedManager);
...
export function updateFocusedModel(camera) {
  ...
  const offset = isMobile ? new THREE.Vector2(0, 0.5) : new THREE.Vector2(-0.7, 0.25); // -x: sinistra, +y: alto , gestisce in modo siverso per mobile e desktop
